...
### 6.1 Tailwind設定の刷新（CSS変数化）
*   **目的**: テーマごとの配色・フォント変更を容易にする。
*   **実装**:
    *   `globals.css` に `:root` でCSS変数を定義（例: `--color-primary`, `--font-heading`）。
    *   `tailwind.config.ts` でこれらを参照するように変更。
    *   テーマ切り替えは `[data-theme="..."]` 属性でCSS変数を上書きすることで実現。

### 6.2 ルーティングの一時リセット
*   **目的**: 旧コンポーネント（`_legacy`）への依存を物理的に断ち切り、新デザインをクリーンな状態で実装する。
*   **手順**:
    *   `app/page.tsx`, `app/[slug]/page.tsx` 等を一時的にシンプルな「Under Construction」コンポーネントに置き換える。
    *   これにより、ビルドエラーを回避しつつ、裏側で新コンポーネント（`core`, `modules`）の開発に集中できる。
