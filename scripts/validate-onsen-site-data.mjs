...
(data.onsens || []).forEach((onsen, index) => {
  ...
  if (!Array.isArray(onsen.gallery) || onsen.gallery.length < 2) fail(`${base}.gallery must contain at least two images`);
  (onsen.gallery || []).forEach((image, imageIndex) => validateImage(image, `${base}.gallery[${imageIndex}]`));
  requireStringArray(onsen.features, `${base}.features`);
  requireStringArray(onsen.seasonal?.map((item) => `${item.season}: ${item.text}`), `${base}.seasonal`);
  requireString(onsen.editorialNote, `${base}.editorialNote`);
  if (!Array.isArray(onsen.benefits) || onsen.benefits.length === 0 || onsen.benefits.some((item) => !item?.label || !item?.text)) fail(`${base}.benefits must contain labeled entries`);
