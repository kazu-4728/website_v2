    ...
    <article className="group overflow-hidden rounded-[1.75rem] border border-[#d8c8ae] bg-[#fffdf8] shadow-[0_16px_50px_rgba(74,55,35,0.08)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_22px_60px_rgba(74,55,35,0.16)]">
      <Link href={`/onsens/${onsen.slug}`} className="block focus:outline-none focus:ring-4 focus:ring-inset focus:ring-[#c28d55]/70">
        <div className={`relative overflow-hidden ${compact ? 'aspect-[16/10]' : 'aspect-[4/3]'}`}>
          <Image src={onsen.image.src} alt={onsen.image.alt} fill priority={priority} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="object-cover transition-transform duration-700 group-hover:scale-105" />
          ...
          <div className="absolute bottom-4 left-4 right-4"><p className="text-xs font-bold tracking-[0.18em] text-[#f2d596]">{onsen.kind}</p><h3 className="mt-1 font-serif text-3xl font-bold leading-tight text-white drop-shadow-sm">{onsen.name}</h3><p className="mt-2 text-xs font-bold text-[#fffaf0]">画像 {onsen.gallery.length}枚・泉質 {onsen.springTypes.filter((item) => !item.includes('詳細')).join(' / ') || '公式情報を確認'}</p></div>
        </div>
      </Link>
      <div className="space-y-5 p-5 md:p-6">
        ...
        <div className="space-y-2 rounded-2xl border border-[#eadfce] bg-[#fffaf0] p-4"><div className="flex items-center justify-between gap-3"><p className="text-xs font-bold tracking-[0.14em] text-[#8c6846]">周辺施設・探し方</p><span className="text-xs font-bold text-[#76624f]">3 links</span></div><div className="grid gap-2">{onsen.facilities.slice(0, 3).map((facility) => <div key={facility.name} className="flex items-center justify-between gap-2 text-sm"><span className="truncate font-semibold text-[#4b4036]">{facility.name}</span><div className="flex shrink-0 gap-2"><Link href={facility.url} target="_blank" rel="noopener noreferrer" className="portal-text-link">案内</Link><Link href={facility.mapUrl} target="_blank" rel="noopener noreferrer" className="portal-text-link">地図</Link></div></div>)}</div></div>
        <div className="flex flex-col gap-2 text-xs text-[#76624f]"><span>情報確認日: {onsen.verifiedAt}</span><ImageCredit image={onsen.image} tone="light" /></div>
      </div>
    </article>
  );
}
