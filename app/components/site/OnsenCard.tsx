    ...
    <article className="group overflow-hidden rounded-[1.75rem] border border-[#d8c8ae] bg-[#fffdf8] shadow-[0_16px_50px_rgba(74,55,35,0.08)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_22px_60px_rgba(74,55,35,0.16)]">
      <Link href={`/onsens/${onsen.slug}`} className="block focus:outline-none focus:ring-4 focus:ring-inset focus:ring-[#c28d55]/70">
        <div className={`relative overflow-hidden ${compact ? 'aspect-[16/10]' : 'aspect-[4/3]'}`}>
          {onsen.imageVerified !== false ? <Image src={onsen.image.src} alt={onsen.image.alt} fill priority={priority} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="object-cover transition-transform duration-700 group-hover:scale-105" /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_10%,rgba(239,208,146,0.38),transparent_34%),linear-gradient(135deg,#31565a,#1f302e_55%,#4b3829)]" aria-hidden="true" />}
          ...
          <div className="absolute bottom-4 left-4 right-4"><p className="text-xs font-bold tracking-[0.18em] text-[#f2d596]">{onsen.kind}</p><h3 className="mt-1 font-serif text-3xl font-bold leading-tight text-white drop-shadow-sm">{onsen.name}</h3><p className="mt-2 text-xs font-bold text-[#fffaf0]">{onsen.imageVerified === false ? '画像を確認・差し替え中' : `画像 ${onsen.gallery.length}枚`}・泉質 {onsen.springTypes.filter((item) => !item.includes('詳細')).join(' / ') || '公式情報を確認'}</p></div>
        </div>
      </Link>
      <div className="space-y-5 p-5 md:p-6">
        ...
        <div className="flex flex-col gap-2 text-xs text-[#76624f]"><span>情報確認日: {onsen.verifiedAt}</span>{onsen.imageVerified !== false && <ImageCredit image={onsen.image} tone="light" />}</div>
      </div>
    </article>
  );
}
