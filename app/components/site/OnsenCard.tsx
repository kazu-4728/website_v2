    ...
    <article className="group overflow-hidden rounded-[1.75rem] border border-[#d8c8ae] bg-[#fffdf8] shadow-[0_16px_50px_rgba(74,55,35,0.08)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_22px_60px_rgba(74,55,35,0.16)]">
      <Link href={`/onsens/${onsen.slug}`} className="block focus:outline-none focus:ring-4 focus:ring-inset focus:ring-[#c28d55]/70">
        <div className={`relative overflow-hidden ${compact ? 'aspect-[16/10]' : 'aspect-[4/3]'}`}>
          ...
          <div className="absolute bottom-4 left-4 right-4"><p className="text-xs font-bold tracking-[0.18em] text-[#f2d596]">{onsen.kind}</p><h3 className="mt-1 font-serif text-3xl font-bold leading-tight text-white drop-shadow-sm">{onsen.name}</h3><p className="mt-2 text-xs font-bold text-[#fffaf0]">{media.status === 'source-pending' ? '画像を確認・差し替え中' : media.status === 'verified-local' ? '検証済みの地域・施設画像' : `画像 ${media.gallery.length}枚`}・泉質 {onsen.springTypes.filter((item) => !item.includes('詳細')).join(' / ') || '公式情報を確認'}</p></div>
        </div>
      </Link>
