...
export function OnsenGallery({ images, name }: OnsenGalleryProps) {
  ...
  const hasSecondaryImages = gallery.length > 1;
  if (!gallery.length) return null;
  return (
    <section aria-label={`${name}の画像ギャラリー`} className="space-y-3">
      <div className={`grid gap-3 ${hasSecondaryImages ? 'md:grid-cols-[1.35fr_0.65fr] md:grid-rows-2' : ''}`}>
        <div className={`relative min-h-[300px] overflow-hidden rounded-[1.5rem] bg-[#221a14] ${hasSecondaryImages ? 'md:row-span-2 md:min-h-[450px]' : 'md:min-h-[450px]'}`}>
