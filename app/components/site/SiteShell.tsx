      ...
      <header className="sticky top-0 z-50 border-b border-stone-200/80 bg-[#f7f3ec]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-8">
          <Link href="/" className="group inline-flex flex-col leading-none">
            <span className="font-serif text-2xl font-bold tracking-tight text-stone-950 md:text-3xl">
              {siteName}
            </span>
            <span className="mt-1 text-xs font-medium tracking-[0.24em] text-stone-500">
              {tagline}
            </span>
          </Link>
          ...
          <nav className="hidden items-center gap-7 md:flex" aria-label="主要ナビゲーション">
            {navigation.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="text-sm font-semibold tracking-wide text-stone-700 transition-colors hover:text-stone-950"
              >
                {item.label}
              </Link>
            ))}
          ...
          <Link
            href="/docs"
            className="rounded-full bg-stone-950 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-transform hover:-translate-y-0.5 md:inline-flex"
          >
            温泉地を見る
          </Link>
        </div>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-5 py-14 md:grid-cols-[1.4fr_1fr] md:px-8">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-7 text-stone-300">
              写真・泉質・アクセス・過ごし方から、関東近郊の温泉地を比較できるJSON駆動型の温泉紹介サイトです。
            </p>
          </div>
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <Link href="/sitemap.xml" className="text-stone-300 hover:text-white">
              サイトマップ
            </Link>
          </div>
        </div>
