...
export function SplitFeature({ data }: Props) {
  ...
  return (
    <section className="relative py-16 md:py-24 overflow-hidden bg-gradient-to-b from-cloud-white via-mist to-cloud-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12">
        <div className={`flex flex-col lg:flex-row items-center gap-8 md:gap-12 lg:gap-16 ${isRight ? '' : 'lg:flex-row-reverse'}`}>
          
          {/* Content Side */}
          <div className="flex-1 space-y-4 md:space-y-6">
            {data.chapter && (
              <div className="flex items-center gap-4 mb-4">
                <span className="text-4xl sm:text-5xl md:text-6xl font-bold text-gray-300 select-none">
                  {data.chapter}
                </span>
            ...
            <div className="bg-white/90 md:backdrop-blur-sm p-4 sm:p-6 md:p-8 rounded-xl shadow-lg">
              {data.subtitle && (
            ...
            {data.stats && (
              <div className="grid grid-cols-2 gap-4 md:gap-6 py-4 md:py-6 border-y border-gray-200 bg-white/80 md:backdrop-blur-sm px-4 md:px-6 rounded-lg">
                {data.stats.map((stat, i) => (
                  <div key={i} className="group">
                    <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-1 group-hover:text-primary-600 transition-colors duration-300">{stat.value}</div>
            ...
            {data.quote && (
              <blockquote className="border-l-4 border-primary-500 pl-4 md:pl-6 py-3 italic text-gray-900 font-medium bg-white/80 rounded-r-lg shadow-sm">
                <p className="text-sm md:text-base lg:text-lg tracking-wide">"{data.quote.text}"</p>
            ...
            {data.link && (
              <Link href={data.link.href} className="inline-block group mt-2 min-h-[44px] min-w-[44px]">
                <span className="text-primary-600 hover:text-primary-700 font-medium text-base md:text-lg transition-colors duration-300 flex items-center">
                  {data.link.text}
