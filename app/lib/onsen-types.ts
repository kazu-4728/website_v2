...
export interface OnsenAccess {
  ...
  fromTokyo: {
    ...
    byCar?: {
      time: number; // minutes
      distance: number; // km
      icName?: string; // Nearest Interchange
      description: string;
    };
    /** 高速バスでのアクセス (By express bus) */
    byBus?: {
      time: number;
      terminal: string; // Departure terminal (e.g. Shinjuku)
      description: string;
    };
  };
...
export interface NearbySpot {
  name: string;
  category: 'sightseeing' | 'gourmet' | 'activity' | 'souvenir' | 'other';
  description: string;
  image?: string; // Image key
  distance?: string; // e.g. "徒歩5分", "車で10分"
  access?: string;
  url?: string;
  mapLink?: string;
}
...
export interface OnsenMeta {
  ...
  updatedAt: string;
  /** APIから取得したデータか (Is fetched from API) */
